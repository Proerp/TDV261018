...
namespace TotalDAL.Helpers.SqlProgrammability.Accounts
{
    public class AccountInvoice
    {
        ...
        public AccountInvoice(TotalSmartPortalEntities totalSmartPortalEntities)
        {
        ...
        private void GetAccountInvoiceIndexes()
        {
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("GetAccountInvoiceIndexes", queryString);
        }
        ...
        private void GetAccountInvoiceViewDetails()
        {
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("GetAccountInvoiceViewDetails", queryString);
        }
        ...
        private void GetPendingGoodsIssues()
        {
            ...
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";
            queryString = queryString + "       SELECT          TOP 200 GoodsIssues.CustomerID, Customers.Code AS CustomerCode, Customers.Name AS CustomerName, Receivers.CustomerID AS ReceiverID, Receivers.Code AS ReceiverCode, Receivers.Name AS ReceiverName, Receivers.VATCode AS ReceiverVATCode, Receivers.AttentionName AS ReceiverAttentionName, Receivers.Telephone AS ReceiverTelephone, Receivers.BillingAddress AS ReceiverBillingAddress, ReceiverEntireTerritories.EntireName AS ReceiverEntireTerritoryEntireName, " + "\r\n";
            queryString = queryString + "                       GoodsIssues.GoodsIssueID, GoodsIssues.Reference AS GoodsIssueReference, GoodsIssues.Code AS GoodsIssueCode, GoodsIssues.EntryDate AS GoodsIssueEntryDate, Customers.Code AS GoodsIssueCustomerCode, Customers.Name AS GoodsIssueCustomerName, GoodsIssues.PaymentTermID, GoodsIssues.TradePromotionID, TradePromotions.Specs AS TradePromotionSpecs, GoodsIssues.TradeDiscountRate, GoodsIssues.VATPercent, GoodsIssues.Description, GoodsIssues.Remarks " + "\r\n";            

            queryString = queryString + "       FROM            GoodsIssues " + "\r\n";
            queryString = queryString + "                       INNER JOIN Customers ON GoodsIssues.GoodsIssueID IN (SELECT GoodsIssueID FROM GoodsIssueDetails WHERE Approved = 1 AND LocationID = @LocationID AND EntryDate > DATEADD(day, -50, GetDate()) AND ROUND(Quantity - QuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0) AND GoodsIssues.CustomerID = Customers.CustomerID " + "\r\n";
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("GetPendingGoodsIssues", queryString);
        }
        ...
        private void GetPendingGoodsIssueConsumers()
        {
            ...
            queryString = queryString + "                       INNER JOIN (SELECT DISTINCT CustomerID, TradePromotionID, TradeDiscountRate, " + (GlobalEnums.VATbyRow ? "0.0 AS" : "") + " VATPercent FROM GoodsIssues WHERE GoodsIssueID IN (SELECT GoodsIssueID FROM GoodsIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND (ROUND(Quantity - QuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0))) PENDINGCustomers ON Customers.CustomerID = PENDINGCustomers.CustomerID " + "\r\n";
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("GetPendingGoodsIssueConsumers", queryString);
        }
        ...
        private void GetPendingGoodsIssueReceivers()
        {
            ...
            queryString = queryString + "       FROM            Customers " + "\r\n";
            queryString = queryString + "                       INNER JOIN (SELECT DISTINCT CustomerID, ReceiverID, TradePromotionID, TradeDiscountRate, " + (GlobalEnums.VATbyRow ? "0.0 AS" : "") + " VATPercent FROM GoodsIssues WHERE GoodsIssueID IN (SELECT GoodsIssueID FROM GoodsIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND (ROUND(Quantity - QuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0 OR ROUND(FreeQuantity - FreeQuantityInvoice, " + (int)GlobalEnums.rndQuantity + ") > 0))) PENDINGReceivers ON Customers.CustomerID = PENDINGReceivers.CustomerID " + "\r\n";
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("GetPendingGoodsIssueReceivers", queryString);
        }
        ...
        private void GetPendingGoodsIssueDetails()
        {
            ...
            queryString = " @AccountInvoiceID Int, @LocationID Int, @GoodsIssueID Int, @CustomerID Int, @ReceiverID Int, @TradePromotionID int, @VATPercent decimal(18, 2), @CommodityTypeID int, @AspUserID nvarchar(128), @FromDate DateTime, @ToDate DateTime, @GoodsIssueDetailIDs varchar(3999), @IsReadonly bit " + "\r\n";
            queryString = queryString + " WITH ENCRYPTION " + "\r\n";
            queryString = queryString + " AS " + "\r\n";
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("GetPendingGoodsIssueDetails", queryString);
        }
        ...
        private string GetPGIDsBuildSQLNew(bool isGoodsIssueID, bool isCustomerID, bool isReceiverID, bool isCommodityTypeID, bool isGoodsIssueDetailIDs)
        {
            ...
            queryString = queryString + "                   INNER JOIN GoodsIssues ON GoodsIssueDetails.GoodsIssueID = GoodsIssues.GoodsIssueID " + (!isGoodsIssueID ? " AND (GoodsIssues.TradePromotionID = @TradePromotionID OR (GoodsIssues.TradePromotionID IS NULL AND @TradePromotionID IS NULL)) " : "") + "\r\n";
        ...
        private void AccountInvoiceSaveRelative()
        {
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("AccountInvoiceSaveRelative", queryString);
        }
        ...
        private void AccountInvoicePostSaveValidate()
        {
            ...
            this.totalSmartPortalEntities.CreateProcedureToCheckExisting("AccountInvoicePostSaveValidate", queryArray);
        }
        ...
        private void AccountInvoiceInitReference()
        {
            ...
            this.totalSmartPortalEntities.CreateTrigger("AccountInvoiceInitReference", simpleInitReference.CreateQuery());
        }
        ...
        private void AccountInvoiceSheet()
        {
            ...
            queryString = queryString + "       SELECT          AccountInvoices.AccountInvoiceID, AccountInvoices.EntryDate, AccountInvoices.Reference, AccountInvoices.Code AS AccountInvoiceCode, AccountInvoices.VATInvoiceNo, AccountInvoices.VATInvoiceDate, AccountInvoices.VATInvoiceSeries, AccountInvoices.CustomerPO, " + "\r\n";
            ...
            queryString = queryString + "                       AccountInvoiceCollections.AccountInvoiceDetailID, AccountInvoiceCollections.IsFreebie, Commodities.CommodityID, Commodities.Code, Commodities.CodePartA, Commodities.CodePartB, ISNULL(Commodities.CodePartA + CASE WHEN @CustomerCategoryID = 999 THEN '' ELSE ' ' + Commodities.CodePartB END, AccountInvoiceCollections.LineDescription) AS LineDescription, CommoditySKU.CodeSKU, Commodities.OfficialName, Commodities.SalesUnit, " + "\r\n";
            ...
            this.totalSmartPortalEntities.CreateStoredProcedure("AccountInvoiceSheet", queryString);
        }
