...
namespace TotalDAL.Repositories.Commons
{
    public class InventoryRepository : IInventoryRepository
    {
        ...
        public InventoryRepository(TotalSmartPortalEntities totalSmartPortalEntities)
        {
            this.totalSmartPortalEntities = totalSmartPortalEntities;
            this.totalSmartPortalEntities.Database.CommandTimeout = 300;
        }

        public bool CheckOverStock(DateTime? checkedDate, string warehouseIDList, string commodityIDList)
        {
            List<OverStockItem> overStockItems = this.totalSmartPortalEntities.GetOverStockItems(checkedDate).ToList();
